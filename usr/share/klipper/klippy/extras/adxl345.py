...
class AccelQueryHelper:
    ...
    def finish_measurements(self):
        ...
        toolhead.wait_moves()
        self.cconn.finalize()
    def update_request_start_time(self, print_time):
        self.request_start_time = print_time
    def update_request_end_time(self, print_time):
...
class ADXL345:
    def __init__(self, config):
        ...
        self.sample_rate = 4000.0 # 采样率800Hz
        ...
        for params in raw_samples:
            ...
            for i in range(len(d) // BYTES_PER_SAMPLE):
                ...
                if 1:
                    # 根据采样率取数据
                    cur_data_time = ptime 
                    if (cur_data_time - self.last_data_time) > 1.0 / self.sample_rate:
                        # logging.info("data_time_delta %f - %f = %f", cur_data_time, self.last_data_time, cur_data_time - self.last_data_time)
                        self.last_data_time = cur_data_time
    ...
    def _finish_measurements(self):
        if not self.is_measuring():
            logging.info("ADXL345 is_measuring\n")
            return
        # Halt bulk reading
    ...
    def _api_update(self, eventtime):
        ...
        with self.lock:
            ...
            # logging.info('raw_samples.length %d', len(raw_samples))
            self.raw_samples = []
        if not raw_samples:
            return {}
        samples = self._extract_samples(raw_samples)
        logging.info('samples.length %d', len(samples))
        if not samples:
            return {}
