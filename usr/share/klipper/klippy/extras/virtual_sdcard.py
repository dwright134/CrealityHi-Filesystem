...
class VirtualSD:
    ...
    def do_pause(self):
        if self.work_timer is not None:
            ...
            while self.work_timer is not None and not self.cmd_from_sd:
                pause_wait_count +=1
                if pause_wait_count > 1000:
                    pause_wait_count = 0
                    logging.info(f'VirtualSD do_pause wait count:{pause_wait_count},is_cancel:{self.is_cancel},cmd_from_sd:{self.cmd_from_sd}')
        ...
        try:
            ...
            with io.open(file_path, "r", encoding="utf-8") as f:
                ...
                while True:
                    ...
                    logging.info("gcode line:%s" % line)
                    ...
                    if not result["E"] and "E" in line:
                        for obj in line_list:
                            if obj.startswith("E"):
                                ...
                                logging.info("power_loss getXYZET E:%s" % result["E"])
    ...
    def get_gcode_wipe_tower_posy(self):
        wipe_tower_posy = None
        try:
            wipe_tower_posy = self.gcode_metadata.get("metadata").get("wipe_tower_posy", None)
        except Exception as err:
            logging.error(err)
        return wipe_tower_posy
        
    def resume_print_speed(self):
        if self.slow_print == True:
            self.slow_print = False
            self.slow_count = 0
            self.gcode.run_script_from_command("M220 S100") # 第一步先恢复到100%
            try:
    ...
    def record_power_loss_info(self,power_loss_switch, bl24c16f,eepromState, gcode_move, start_time, end_time, interval_start_time, interval_end_time):
        try:
            ...
            # 没有层信息的话 5秒写一次EEPROM
            state = False
            if self.layer > 2 and self.layer>self.last_layer:
                state = True
            elif self.layer == 0:
            ...
            if power_loss_switch and bl24c16f and (self.layer > 2 or (self.count_G1 > 18 and gcode_move.last_position[2] > 0.6)) and state and self.file_position>0:
                logging.info("record_power_loss_info to eeprom layer:%s" % self.layer)
        ...
        if power_loss_switch and bl24c16f and (self.layer > 2 or gcode_move.last_position[2] > 3) and self.current_file and interval_end_time-interval_start_time > 15:
    ...
    def work_handler(self, eventtime):
        ...
        # 如果do_pause先执行，这里调用run_script就会锁住
        if not self.must_pause_work:
            ...
            if power_loss_switch and bl24c16f and self.current_file:
                # Prevent the client from sending the SET_PRESSURE_ADVANCE command before the print_file_name_path file is created.
                pressure_advance_cmd = ""
                extruder = self.printer.lookup_object('extruder',default=None)
                if extruder:
                    pressure_advance = extruder.get_status(eventtime=None).get("pressure_advance")
                    smooth_time = extruder.get_status(eventtime=None).get("smooth_time")
                    pressure_advance_cmd = f'SET_PRESSURE_ADVANCE ADVANCE={pressure_advance} SMOOTH_TIME={smooth_time}'
                    logging.info(f'work_handler start print_13_1 pressure_advance_cmd:{pressure_advance_cmd}')
                gcode_move.recordPrintFileName(self.print_file_name_path, self.current_file.name, pressure_advance=pressure_advance_cmd)
        ...
        if box_obj:
            ...
            extruder_filament = box_obj.box_action.get_filament_sensor_detect()
