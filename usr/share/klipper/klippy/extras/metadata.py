...
class BaseSlicer(object):
    ...
    def parse_wipe_tower_posy(self) -> Optional[float]:
        return None
...
class PrusaSlicer(BaseSlicer):
    ...
    def parse_flush_para(self) -> Optional[Dict[str, List[int]]]:
        ...
        nozzle_volume_match = re.search(r'nozzle_volume\s*=\s*([\d.]+)', self.footer_data)
        if not nozzle_volume_match:
            nozzle_volume = re.search(r'nozzle_volume\s*=\s*([\d.]+)', self.header_data)
        if nozzle_volume_match:
            nozzle_volume = float(nozzle_volume_match.group(1))

        # return
        if flush_multiplier is not None or flush_volumes_matrix is not None or nozzle_volume is not None:
            return {'flush_multiplier': flush_multiplier, 'flush_volumes_matrix': flush_volumes_matrix, 'nozzle_volume': nozzle_volume}
        else:
            return None
        
    def parse_wipe_tower_posy(self) -> Optional[float]:
        return _regex_find_first(
        r"; wipe_tower_y = (\d+\.?\d*)", self.footer_data)
