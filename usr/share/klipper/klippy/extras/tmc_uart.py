...
class MCU_TMC_uart:
    ...
    def _do_get_register(self, reg_name):
        ...
        # self.gcode.respond_info("  reg_name:" + str(reg_name))
        if self.printer.get_start_args().get('debugoutput') is not None:
            return 0
        for retry in range(5):
            val = self.mcu_uart.reg_read(self.instance_id, self.addr, reg)
            # self.gcode.respond_info("self.instance_id:" + str(self.instance_id) + "  self.addr:" + str(self.addr)  + "  reg:" + str(reg) + "  val:" + str(val))
            if val is not None:
                return val
    ...
    def test_get_register(self, reg):
        for retry in range(5):
            val = self.mcu_uart.reg_read(self.instance_id, self.addr, reg)
            # self.gcode.respond_info("self.instance_id:" + str(self.instance_id) + "  self.addr:" + str(self.addr)  + "  reg:" + str(reg) + "  val:" + str(val))
            if val is not None:
                return val
        raise self.printer.command_error(
            "Unable to read tmc uart register %s" % (reg))
        pass
