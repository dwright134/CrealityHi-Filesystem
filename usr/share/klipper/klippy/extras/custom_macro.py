...
class CUSTOM_MACRO:
    def __init__(self, config):
        ...
        self.gcode.register_command("SET_RMODE_FLAG", self.cmd_SET_RMODE_FLAG, desc=self.cmd_SET_RMODE_FLAG_help)
        pass
    ...
    cmd_SET_RMODE_FLAG_help = "set rmode flag"
    def cmd_SET_RMODE_FLAG(self, gcmd):
        self.rmode_flag =  gcmd.get_int('FLAG', default=1, minval=0, maxval=1)
        gcmd.respond_info("[cmd_SET_RMODE_FLAG] self.rmode_flag={}".format(self.rmode_flag))
        import json, logging
        try:
            print_stats = self.printer.lookup_object('print_stats')
            v_sd = self.printer.lookup_object('virtual_sdcard')
            speed_mode_path = v_sd.speed_mode_path
            if print_stats.state == "printing" or print_stats.state == "paused":
                result = {}
                if self.rmode_flag == 1:
                    result["speed_mode"] = 3
                elif self.rmode_flag == 0:
                    result["speed_mode"] = -1
                with open(speed_mode_path, "w") as f:
                    f.write(json.dumps(result))
                    f.flush()
