...
class PrinterExtruder:
    ...
    def check_move(self, move):
        ...
        if not self.heater.can_extrude:
            if self.force_move_e_flag == False:
    ...
    def cmd_force_move_e(self, gcmd):
        gcode = self.printer.lookup_object('gcode')
        toolhead = self.printer.lookup_object('toolhead')
        gcode_move = self.printer.lookup_object('gcode_move')
        pos = toolhead.get_position()
        gcmd.respond_info("Forcing move E: %s" % (str(pos)))
        self.force_move_e_flag = True
        # pos_old = pos[3]
        e_len = gcmd.get_float('E', 0.)
        e_v = gcmd.get_float('F', 300.)
        e_absFlag = gcode_move.absolute_extrude
        gcode.run_script_from_command("M83")
        gcode.run_script_from_command("G1 E%f F%f" % (e_len, e_v))
        gcode.run_script_from_command("G4 S1000")
