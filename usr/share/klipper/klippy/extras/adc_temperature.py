...
class PrinterADCtoTemperature:
    def __init__(self, config, adc_convert):
        ...
        self.adc_temp_list = config.getfloatlist('adc_temp_list', sep=',',default=(25, 100, 180, 200, 220, 300), count=6) # ADC采样温度
        self.actual_temp_list = config.getfloatlist('actual_temp_list', sep=',',default=(25, 100, 180, 200, 220, 300), count=6) # 实际温度
        self.k_slope = [0,0,0,0,0,0]
        for i in range(0,5):
            self.k_slope[i] = ((self.actual_temp_list[i + 1] - self.actual_temp_list[i])/(self.adc_temp_list[i + 1] - self.adc_temp_list[i]))
            pass
    ...
    def adc_callback(self, read_time, read_value):
        if self.temp_offset_flag == True:
            adc_temp = self.adc_convert.calc_temp(read_value)
            if (adc_temp > self.adc_temp_list[0]) & (adc_temp < self.adc_temp_list[1]):
                actual_temp = (adc_temp - self.adc_temp_list[0]) *self.k_slope[0] + self.actual_temp_list[0]
            elif (adc_temp >= self.adc_temp_list[1]) & (adc_temp < self.adc_temp_list[2]):
                actual_temp = (adc_temp - self.adc_temp_list[1]) *self.k_slope[1] + self.actual_temp_list[1]
            elif (adc_temp >= self.adc_temp_list[2]) & (adc_temp < self.adc_temp_list[3]):
                actual_temp = (adc_temp - self.adc_temp_list[2]) *self.k_slope[2] + self.actual_temp_list[2]
            elif (adc_temp >= self.adc_temp_list[3]) & (adc_temp < self.adc_temp_list[4]):
                actual_temp = (adc_temp - self.adc_temp_list[3]) *self.k_slope[3] + self.actual_temp_list[3]
            elif adc_temp >= self.adc_temp_list[4]:
                actual_temp = (adc_temp - self.adc_temp_list[4]) * self.k_slope[4] + self.actual_temp_list[4]
            elif adc_temp < self.adc_temp_list[0]:
                actual_temp = self.adc_convert.calc_temp(read_value)
            # self.gcode.respond_info("adc_temp:%d,actual_temp:%d"%(adc_temp,actual_temp))
            temp = actual_temp
        else:
